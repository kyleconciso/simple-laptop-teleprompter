...
class TeleprompterGUI:
    ...
    def center_label(self, target_word=None, target_index=None):
        """Center a label around a target word"""
        sentence = self.label.cget("text")
        ...
        font = tk.font.Font(font=self.label.cget("font"))
        target_width = font.measure(target_word)
        left_width = font.measure(" ".join(words[:target_index]))
        self.label.place(x=((self.root.winfo_screenwidth() - target_width) / 2 - left_width)-10, y=-(self.height / 2))
